...
- ctospremote:      Add git remote for matching AOSP repository.
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/ctosp/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/ctosp-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function githubremote()
{
    ...
    local REMOTE=$(git config --get remote.ctosp.projectname)

    local PROJECT=$(echo $REMOTE | sed -e "s#platform/#android/#g; s#/#_#g")

    git remote add github https://github.com/ctosp/$PROJECT
...
function losrepopick() {
    T=$(gettop)
    $T/vendor/ctosp/build/tools/losrepopick.py $@
}
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/ctosp/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
