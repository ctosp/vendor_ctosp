...
function setup_vendor() {
    ...
    export CTOSP_ROOT="$3"
    if [ ! -d "$CTOSP_ROOT" ]; then
        echo "\$CTOSP_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$CTOSP_ROOT/$OUTDIR" ]; then
        mkdir -p "$CTOSP_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$CTOSP_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$CTOSP_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$CTOSP_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$CTOSP_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$CTOSP_ROOT"/prebuilts/tools-custom/common/smali/baksmali.jar
        export SMALIJAR="$CTOSP_ROOT"/prebuilts/tools-custom/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$CTOSP_ROOT"/prebuilts/tools-custom/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$CTOSP_ROOT"/prebuilts/tools-custom/"${HOST,,}"-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$CTOSP_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$CTOSP_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$CTOSP_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$CTOSP_ROOT"/vendor/CTOSP/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$CTOSP_ROOT"/"$OUTDIR"/radio
